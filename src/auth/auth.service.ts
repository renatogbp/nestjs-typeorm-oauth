...
export class AuthService {
  ...
  async validateOAuthLogin(profile: any, provider: Provider): Promise<string> {
    try {
        let user = await this.usersService.findOne({ oAuthLogins: { provider, providerId: profile.id } });
        if (!user) {
        ...
        return sign(payload, this.configService.getJwtSecret(), { expiresIn: 3600 });
    } catch (err) {
  ...
  async validateUser(payload: JwtPayload): Promise<User> {
    const user = await this.usersService.findOne({ _id: ObjectId(payload.sub) });
    return user;
  }
}
